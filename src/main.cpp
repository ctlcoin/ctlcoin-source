...
#if defined(NDEBUG)
# error "ctlcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x09379e8ce1373193a502094fc98f028e373a8f076c3fc0340e79de256098d6f5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ctlcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ctlcoin Signed Message:\n";
...
{
    // ctlcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ctlcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ctlcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1467986405, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "03.6.16";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d8b8268b878dd883f5e8bdcef628e8af0de82378233a60d81398a8cbd4182c851f242a9bf3cd748590b36b73fe21ff93d9b503962dfe028596f048147f46887f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1467986405;
        ...
        assert(block.hashMerkleRoot == uint256("0x35ec39165ff3af3671228bb8e556b3a00c715c897bbdbbdde979a286bffcef20"));
...
unsigned char pchMessageStart[4] = { 0x3c, 0x83, 0x4e, 0xea }; // ctlcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ctlcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ctlcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ctlcoinMiner(CWallet *pwallet)
{
    printf("ctlcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ctlcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ctlcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ctlcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ctlcoinMiner, pwallet));
}
